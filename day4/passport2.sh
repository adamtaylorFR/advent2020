...
do
  ...
  if [[ -z "${line// }" ]]; then
    ...
    if [[ $buffer == *"byr"* ]] && [[ $buffer == *"iyr"* ]] && [[ $buffer == *"eyr"* ]] && [[ $buffer == *"hgt"* ]] && [[ $buffer == *"hcl"* ]] && [[ $buffer == *"ecl"* ]] && [[ $buffer == *"pid"* ]] ; then
      ...
      if [ "$byr" -le 2002 ] && [ "$byr" -ge 1920 ] && [ "$iyr" -le 2020 ] && [ "$iyr" -ge 2010 ] && [ "$eyr" -le 2030 ] && [ "$eyr" -ge 2020 ] && [[ ${#byr} -eq 4 ]] && [[ ${#iyr} -eq 4 ]] && [[ ${#eyr} -eq 4 ]]; then
        echo "years good"
      else
        badvalid="bad"
      fi
      ...
      echo "$hcl"
      if [[ "${hcl:0:1}" == "#" ]] && [[ "${hcl:1:7}" =~ [0-9A-Fa-f] ]] && [[ ${#hcl} -eq 7 ]]; then
         echo "hair good";
      else
         badvalid="bad"
      fi
      ...
      echo "$ecl"
      if  [[ $ecl == *"amb" ]] || [[ $ecl == *"gry" ]] || [[ $ecl == *"blu" ]] || [[ $ecl == *"brn" ]] || [[ $ecl == *"grn" ]] || [[ $ecl == *"hzl" ]] || [[ $ecl == *"oth" ]] ;then
        if [[ ${#ecl} -eq 3 ]] ; then
         echo "eye good";
       else
          badvalid="bad"
...
echo "$north_poll northpoll and $passport passport for $(($north_poll + $passport)) total ($bad bad)"
